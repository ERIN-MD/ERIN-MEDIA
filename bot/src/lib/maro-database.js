...
class Database {
  constructor(dbPath) {
    ...
    this._userCache = new LRUCache({ max: 2000, ttl: 1000 * 60 * 10 });
    this._groupCache = new LRUCache({ max: 500, ttl: 1000 * 60 * 10 });
    this.ensureDir();
  }
  ...
  getUser(jid) {
    ...
    if (this._userCache.has(cleanJid)) return this._userCache.get(cleanJid);
    
    const user = this.db.data.users[cleanJid] || null;
    if (user) this._userCache.set(cleanJid, user);
    return user;
  }
  ...
  setUser(jid, data = {}) {
    ...
    this._userCache.set(cleanJid, user);
    return user;
  }
  ...
  getGroup(jid) {
    if (!jid) return null;
    if (this._groupCache.has(jid)) return this._groupCache.get(jid);
    const group = this.db.data.groups[jid] || null;
    if (group) this._groupCache.set(jid, group);
    return group;
  }
