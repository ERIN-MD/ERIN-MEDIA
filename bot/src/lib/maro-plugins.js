...
import { theme, chalk, logger } from "./maro-logger.js";
/**
...
/**
 * سجل تشخيص التحميل — لا يُخفى أي فشل أو تصادم.
 * @type {{collisions: object[], failures: object[], invalid: object[], loadedAt: number|null}}
 */
...
function resetPluginDiagnostics() {
  pluginDiagnostics.collisions = [];
  pluginDiagnostics.failures = [];
  pluginDiagnostics.invalid = [];
  pluginDiagnostics.loadedAt = null;
}

function getPluginDiagnostics() {
  return {
    ...pluginDiagnostics,
    collisions: [...pluginDiagnostics.collisions],
    failures: [...pluginDiagnostics.failures],
    invalid: [...pluginDiagnostics.invalid],
  };
}
...
import { recordPluginLoadFailure } from "./maro-plugin-diagnostics.js";
...
async function loadPlugin(filePath, bustCache = false) {
  try {
    ...
    // ⚠️ كانت هذه الحالات تُرجع null بصمت فتختفي الإضافة بلا أثر.
    if (!plugin.config || !plugin.handler) {
      pluginDiagnostics.invalid.push({
        file: filePath,
        reason: !plugin.config ? "missing exported config" : "missing exported handler",
      });
      return null;
    }

    if (typeof plugin.handler !== "function") {
      pluginDiagnostics.invalid.push({ file: filePath, reason: "handler is not a function" });
      return null;
    }
    ...
    pluginDiagnostics.failures.push({
      file: filePath,
      reason: error.message.split("\n")[0],
    });
    recordPluginLoadFailure(filePath, error);
    // فشل التحميل خطأ حقيقي ويجب أن يظهر دائماً (كان الشرط هنا ميتاً: `|| true`)
...
function registerPlugin(plugin) {
  ...
  // ⚠️ كان Map.set يستبدل الأوامر المكرّرة بصمت، فتفوز آخر إضافة تُحمَّل
  // وترتيبها يعتمد على نظام الملفات (سلوك غير حتمي بين الأجهزة).
  // الآن نسجّل كل تصادم في سجل التشخيص بدل إخفائه.
  for (const n of names) {
    const existing = pluginStore.commands.get(n);
    if (existing && existing !== plugin) {
      pluginDiagnostics.collisions.push({
        type: "command",
        key: n,
        keptFile: plugin.filePath || "(unknown)",
        shadowedFile: existing.filePath || "(unknown)",
      });
    }
  ...
  for (const a of aliases) {
    if (pluginStore.commands.has(a) && !names.includes(a)) {
      pluginDiagnostics.collisions.push({
        type: "alias-shadows-command",
        key: a,
        keptFile: plugin.filePath || "(unknown)",
        shadowedFile: pluginStore.commands.get(a)?.filePath || "(unknown)",
      });
    }
    const existingAlias = pluginStore.aliases.get(a);
    if (existingAlias && existingAlias !== primaryName) {
      pluginDiagnostics.collisions.push({
        type: "alias",
        key: a,
        keptFile: plugin.filePath || "(unknown)",
        shadowedCommand: existingAlias,
      });
    }
 ...
 * @example
 * const count = loadPlugins('./maro-plugins');
...
async function loadPlugins(pluginsDir) {
  ...
  // ── تقرير التشخيص: لا تصادم صامت ولا فشل مخفي ────────────────
  pluginDiagnostics.loadedAt = Date.now();
  const { collisions, failures, invalid } = pluginDiagnostics;
  if (failures.length) {
    logger.error("plugin", `فشل تحميل ${failures.length} ملف — راجع .تشخيص`);
  }
  if (invalid.length) {
    logger.warn("plugin", `${invalid.length} ملف بلا config/handler صالح`);
  }
  if (collisions.length) {
    const keys = [...new Set(collisions.map((c) => c.key))];
    logger.warn(
      "plugin",
      `${collisions.length} تصادم في ${keys.length} مفتاح (يفوز آخر تحميل) — راجع .تشخيص`,
    );
  }
  return loadedCount;
}
