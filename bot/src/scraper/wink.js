// ═══════════════════════════════════════════════
// 📁 src/scraper/wink-enhancer.js
// ✨ Wink AI - تحسين جودة الفيديو بالذكاء الاصطناعي
// ═══════════════════════════════════════════════
...
const CLIENT_LANGUAGE = "ar_EG";
const CLIENT_TIMEZONE = "Africa/Cairo";
...
async function getMaatSign() {
  ...
  if (res.status >= 400 || res.data?.code !== 0) {
    throw new Error(`فشل get_maat_sign: ${JSON.stringify(res.data)}`);
  }
...
async function getUploadPolicy(sign) {
  ...
  if (res.status >= 400 || !Array.isArray(res.data) || !res.data[0]?.qiniu) {
    throw new Error(`فشل upload policy: ${JSON.stringify(res.data)}`);
  }
...
async function getVideoInfo(fileKey) {
  ...
  if (res.status >= 400 || res.data?.code !== 0) {
    throw new Error(`فشل معلومات الفيديو: ${JSON.stringify(res.data)}`);
  }
...
async function startTranscode(fileKey) {
  ...
  if (res.status >= 400 || res.data?.code !== 0 || !res.data?.data?.id) {
    throw new Error(`فشل بدء التحويل: ${JSON.stringify(res.data)}`);
  }
...
async function queryTranscode(id) {
  ...
  if (res.status >= 400 || res.data?.code !== 0) {
    throw new Error(`فشل استعلام التحويل: ${JSON.stringify(res.data)}`);
  }
...
async function waitTranscode(id, fallbackSourceUrl, maxTry = 80, delayMs = 3000) {
  ...
  for (let i = 1; i <= maxTry; i++) {
    ...
    const videoTranscoded =
      data?.video_transcoded || data?.transcoded_video || data?.transcoded_url || data?.video_url || "";
...
async function delivery(sourceUrl, videoTranscoded, taskName) {
  ...
  if (res.status >= 400 || res.data?.code !== 0) {
    throw new Error(`فشل التسليم: ${JSON.stringify(res.data)}`);
  }
...
async function queryBatch(msgId) {
  ...
  if (res.status >= 400 || res.data?.code !== 0) {
    throw new Error(`فشل استعلام الدفعة: ${JSON.stringify(res.data)}`);
  }
...
async function waitResult(firstMsgId, maxTry = 120, delayMs = 5000) {
  ...
  for (let i = 1; i <= maxTry; i++) {
    ...
    if (errorCode && errorCode !== 29901 && errorCode !== 0) {
      throw new Error(`فشل المهمة: ${errorCode}`);
    }

    await sleep(delayMs);
  }

  throw new Error(`النتيجة لم تكتمل بعد`);
}

async function winkEnhance(video, options = {}) {
  if (!video) throw new Error("الفيديو مطلوب");

  const safeName = options.filename || `wink-${crypto.randomUUID()}.mp4`;
  ...
  try {
    const taskName = `محسن-فائق الدقة-${path.parse(filePath).name}`;
    ...
    if (!firstMsgId) {
      throw new Error(`لم يتم الحصول على msg_id`);
    }

    const resultUrl = await waitResult(
      firstMsgId,
      options.maxResultTry,
      options.delayMs,
    );
