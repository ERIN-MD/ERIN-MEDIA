...
async function handler(m, { sock }) {
    ...
    if (text.length > 100) {
        return m.reply(`❌ النص طويل! الحد الأقصى 100 حرف.`)
    }
    ...
    try {
        ...
        if (!data?.status || !data?.data?.url) {
            throw new Error('API لم يرجع بيانات صالحة')
        }

        const stickerUrl = data.data.url
        const stickerRes = await f(stickerUrl, 'buffer')
        if (!stickerRes) throw new Error('فشل تحميل الملصق')
