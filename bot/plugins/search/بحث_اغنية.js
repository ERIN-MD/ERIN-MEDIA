...
async function _routePreview(query, sessionCookie = "") {
  ...
  if (res.status !== 200) throw new Error(`فشل الطلب`);
...
const pluginConfig = {
  ...
  alias: ["sts"],
  category: "search",
  description: "بحث عن أغنية من كلماتها",
  usage: ".بحث_اغنية <كلمات>",
  example: ".بحث_اغنية ku menangis",
...
async function handler(m, { sock }) {
  ...
  if (!query) {
    return m.reply(`🎵 *بحث أغنية*\n\n📌 مثال: \`${m.prefix}بحث_اغنية ku menangis\``);
  }
  ...
  try {
    ...
    if (!result.song) {
      await m.react("❌");
      return m.reply("⚠️ لم يتم العثور على الأغنية. جرب كلمات أخرى.");
    }

    let caption = `🎵 *نتيجة البحث*\n\n`;
    caption += `*الأغنية:* ${result.song}\n`;
    if (result.artist) caption += `*الفنان:* ${result.artist}\n`;
    if (result.album) caption += `*الألبوم:* ${result.album}\n`;
    if (result.year) caption += `*السنة:* ${result.year}\n`;
    if (result.genre) caption += `*النوع:* ${result.genre}\n`;
    if (result.youtubeUrl) caption += `*يوتيوب:* ${result.youtubeUrl}\n`;
    ...
    if (result.lyrics) {
      await sock.sendMessage(m.chat, { text: `*الكلمات:*\n\n${result.lyrics}` }, { quoted: m });
    }
