...
const pluginConfig = {
    ...
    alias: ['ephoto'],
    category: 'ephoto',
    description: 'إنشاء تأثيرات نصية رائعة بمختلف الأنماط',
    usage: '.<تأثير> <نص>',
    example: '.glitchtext Maro-AI',
    isOwner: false,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
async function handler(m, { sock }) {
    ...
    if (command === 'ephoto') {
        ...
        return m.reply(
            `🎨 *تأثيرات إيفوتو*\n\n` +
            `> اصنع تأثيرات نصية رائعة!\n\n` +
            `╭┈┈⬡「 📋 *قائمة التأثيرات* 」\n${effectList}\n╰┈┈┈┈┈┈┈┈⬡\n\n` +
            `> *مثال:* ${m.prefix}glitchtext Maro-AI`
        )
    }
    
    if (!text) {
        return m.reply(`❌ *خطأ*\n\n> أدخل النص!\n> *مثال:* ${m.prefix}${command} Maro-AI`)
    }
    
    const effectUrl = EFFECT_URLS[command]
    if (!effectUrl) {
        return m.reply(`❌ التأثير غير موجود`)
    }
