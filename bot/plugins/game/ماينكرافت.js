...
import {
  ...
  healPlayer,
  JACKPOT_POOLS,
} from "../../src/lib/maro-minecraft.js";
import {
  biomes,
  travelRequirements,
  pickaxes,
  ...
  SMELT_RECIPES,
  CRAFT_RECIPES,
} from "../../src/lib/maro-minecraft-data.js";
...
let thumbMC = null;
try {
  const p = path.join(process.cwd(), "assets", "images", "maro-minecraft.jpg");
  if (fs.existsSync(p)) thumbMC = fs.readFileSync(p);
} catch (e) {}

function ctx() {
  const sId = config.saluran?.id || "120363400911374213@newsletter";
  const sName = config.saluran?.name || config.bot?.name || "Maro-AI";
  return {
