// تحسين_الصورة_3 - أمر لتحسين جودة الصورة إلى HD (BeautyPlus)
...
const config = {
  ...
  alias: ["hd3"],
  category: "tools",
  description: "تحسين جودة الصورة إلى HD (BeautyPlus)",
  usage: ".تحسين_الصورة_3 (رد على صورة)",
  example: ".تحسين_الصورة_3",
  cooldown: 20,
  energi: 2,
  isEnabled: true,
};
...
async function getPolicy(suffix) {
  ...
  if (res.status !== 200 || !Array.isArray(res.data)) {
    throw new Error("فشل الحصول على السياسة");
  }
...
async function checkQuota(uid) {
  ...
  if (res.status !== 200) {
    throw new Error("فشل التحقق من الحصة");
  }

  if (res.data?.needUpgrade) {
    throw new Error("تحتاج إلى ترقية");
  }
      ...
      for (const match of raw.matchAll(/^data:\s*(.+)$/gm)) {
        try {
          ...
          if (data.status === "failed" || data.status === "error") {
            ...
            clearTimeout(timer);
            res.data.destroy();
            reject(new Error("فشلت المعالجة"));
            return;
          }
        } catch {}
      }
...
async function handler(m, { sock }) {
  ...
  if (!img) {
    return m.reply(
      `*🪄 تحسين الصورة (BeautyPlus)*\n> رد على صورة لتحسينها إلى HD\n\n\`\`\`${m.prefix}تحسين_الصورة_3\`\`\``
    );
  }
  ...
  try {
    ...
    if (!resultUrl) {
      throw new Error("فشل تحسين الصورة، النتيجة فارغة.");
    }
