...
const pluginConfig = {
  ...
  alias: ["infotourney"],
  category: "info",
  description: "معلومات أحدث بطولات Mobile Legends",
  usage: ".بطولات",
  example: ".بطولات",
  isOwner: false,
  isPremium: false,
...
async function getInfoTourney() {
  ...
  $(".items-row .item").each((_, element) => {
    ...
    if (datePublished) { datePublished = moment(datePublished).tz("Asia/Jakarta").format("DD/MM/YYYY HH:mm"); }
    ...
    tournaments.push({ title, imageUrl: new URL(image, url).href, datePublished, description, info, url: new URL(link, url).href });
  });
...
async function handler(m, { sock }) {
  ...
  try {
    ...
    if (!tournaments || tournaments.length === 0) {
      await m.react("❌");
      return m.reply("❌ لا توجد بطولات");
    }

    const saluranId = config.saluran?.id || "120363400911374213@newsletter";
    const saluranName = config.saluran?.name || config.bot?.name || "Maro-AI";

    let text = `🏆 *بطولات Mobile Legends*\n\n`;
    text += `> آخر 5 بطولات\n\n`;

    for (let i = 0; i < tournaments.length; i++) {
      const t = tournaments[i];
      text += `${i + 1}. *${t.title}*\n`;
      text += `📅 ${t.datePublished || "غير معروف"}\n`;
    ...
    if (firstImage) {
      await sock.sendMedia(m.chat, firstImage, text, m, { type: "image" });
    } else {
      await m.reply(text);
    }
