import config from "../../config.js";
import { getDatabase } from "../../src/lib/maro-database.js";
import te from "../../src/lib/maro-error.js";

const pluginConfig = {
  name: "انضمام_تلقائي",
  alias: ["autojoingc"],
  category: "owner",
  description: "انضمام تلقائي للمجموعات من الرابط المكتشف في المحادثة",
  usage: ".انضمام_تلقائي on/off",
  example: ".انضمام_تلقائي on",
  isOwner: true,
  isPremium: false,
  isGroup: false,
  isPrivate: false,
  cooldown: 3,
  energi: 0,
  isEnabled: true,
};

const GROUP_LINK_REGEX = /chat\.whatsapp\.com\/([a-zA-Z0-9]{18,24})/gi;

async function handler(m) {
  const db = getDatabase();
  const arg = (m.args?.[0] || "").toLowerCase();
  if (!arg || !["on", "off"].includes(arg)) {
    const current = db.setting("autoJoinGc") || false;
    return m.reply(
      `🔗 *انضمام تلقائي للمجموعات*\n\nالحالة: *${current ? "مفعل ✅" : "معطل ❌"}*\n\n\`${m.prefix}انضمام_تلقائي on\` — تفعيل\n\`${m.prefix}انضمام_تلقائي off\` — تعطيل`,
    );
  }
  const enabled = arg === "on";
  db.setting("autoJoinGc", enabled);
  await db.save();
  m.reply(
    `${enabled ? "✅" : "❌"} الانضمام التلقائي للمجموعات *${enabled ? "مفعل" : "معطل"}*`,
  );
}

async function autoJoinDetector(m, sock) {
  const db = getDatabase();
  if (!db?.ready) return false;
  if (!db.setting("autoJoinGc")) return false;
  if (!m.body) return false;
  const matches = [...m.body.matchAll(GROUP_LINK_REGEX)];
  if (!matches.length) return false;
  let joined = 0;
  for (const match of matches) {
    const code = match[1];
    try {
      const result = await sock.groupAcceptInvite(code);
      if (result) {
        joined++;
      }
    } catch (e) {
      const msg = e.message || String(e);
      if (msg.includes("already") || msg.includes("participant")) {
        await m.reply(`⚠️ البوت موجود مسبقاً في هذه المجموعة`);
      } else if (msg.includes("expired") || msg.includes("revoked")) {
        await m.reply(`❌ رابط المجموعة منتهي/ملغي`);
      } else {
        await m.reply(te(m.prefix, m.command, m.pushName));
      }
    }
  }
  return joined > 0;
}

export { pluginConfig as config, handler, autoJoinDetector };