...
async function handler(m, { sock, store }) {
    ...
    // ⚠️ أمان: تنفيذ كود عشوائي — يتطلب تفعيلاً صريحاً عبر MAROBOT_DEV_CONSOLE=true
    if (!config.devConsole?.enabled) {
        return m.reply('🔒 *كونسول المطوّر معطّل*\n\nفعّله بـ `MAROBOT_DEV_CONSOLE=true` في ملف .env')
    }
