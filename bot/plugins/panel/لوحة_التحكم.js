// لوحة التحكم - قائمة لوحة تحكم Pterodactyl (v1-v5)

import config from '../../config.js'
import { getRoles, getUserRole, getAccessibleServers, VALID_SERVERS } from '../../src/lib/maro-roles-cpanel.js'

const pluginConfig = {
    name: 'لوحة_التحكم',
    alias: ['cpanel'],
    category: 'panel',
    description: 'قائمة لوحة تحكم Pterodactyl (v1-v5)',
    usage: '.لوحة_التحكم',
    example: '.لوحة_التحكم',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    const userRoleList = userServers.map(s => `${s.server.toUpperCase()}:${s.role}`).join(', ') || 'لا يوجد'
    
    let txt = `🖥️ *قائمة لوحة التحكم v2.0*\n\n`
    txt += `> V1: ${serverStatuses.v1} | V2: ${serverStatuses.v2} | V3: ${serverStatuses.v3} | V4: ${serverStatuses.v4} | V5: ${serverStatuses.v5}\n`
    txt += `> دورك: *${m.isOwner ? 'مالك البوت' : userRoleList}*\n\n`
    ...
    txt += `╭─「 🌊 *خوادم DigitalOcean* 」\n`
    txt += `┃ الحالة: ${doHasToken} Token\n`
    txt += `┃\n`
    txt += `┃ 📦 *إنشاء VPS:*\n`
    ...
    txt += `> _مدعوم من ${config.info?.website || 'MaroAI'}_`
