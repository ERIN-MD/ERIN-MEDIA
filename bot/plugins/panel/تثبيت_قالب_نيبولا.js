...
const pluginConfig = {
    ...
    usage: '.تثبيت_قالب_نيبولا <IP>|<كلمة_المرور>',
    example: '.تثبيت_قالب_نيبولا 192.168.1.1|كلمة_السر',
    isOwner: true,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
    ...
    return new Promise((resolve, reject) => {
        conn.exec(cmd, { pty: true }, (err, stream) => {
            ...
            stream.on('close', (code, signal) => {
                if (code !== 0) return reject(new Error(`فشل الأمر مع الكود ${code}\nالمخرجات: ${output}`))
                resolve(output)
            })
...
function handler(m) {
    ...
    if (!text) {
        return m.reply(
            `╭┈┈⬡「 🌌 *تثبيت قالب نيبولا* 」\n┃ ㊗ طريقة الاستخدام: \`${m.prefix}تثبيت_قالب_نيبولا <IP>|<كلمة_المرور>\`\n╰┈┈⬡\n\n> \`مثال: ${m.prefix}تثبيت_قالب_نيبولا 192.168.1.1|كلمة_السر\``
        )
    }
    ...
    conn.on('ready', async () => {
        try {
            await m.reply(`🕕 *[1/3] تجهيز البيئة...*\n\n> جاري تثبيت Node.js 22، Yarn، والتبعيات...`)
            await execSSH(conn, CMD_DEPS)

            await m.reply(`🕕 *[2/3] تثبيت Blueprint...*\n\n> جاري تحميل وتكوين إطار Blueprint...`)
            await execSSH(conn, CMD_BLUEPRINT)

            await m.reply(`🕕 *[3/3] تثبيت نيبولا...*\n\n> جاري تثبيت قالب نيبولا (تأكيد تلقائي)...`)
            await execSSH(conn, CMD_NEBULA)

            m.react('✅')
            await m.reply(
                `╭┈┈⬡「 ✅ *قالب نيبولا* 」\n┃ ㊗ الحالة: *تم التثبيت*\n┃ ㊗ IP: ${ipvps}\n╰┈┈⬡\n\n> _تم تثبيت قالب نيبولا بنجاح!_`
            )
        } catch (err) {
        ...
        m.react('❌')
        m.reply(`❌ فشل الاتصال!\n\n> IP أو كلمة المرور غير صالحة / الخادم متوقف.`)
