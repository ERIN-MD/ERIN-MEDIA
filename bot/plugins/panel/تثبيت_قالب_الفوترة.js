...
const pluginConfig = {
    ...
    usage: '.تثبيت_قالب_الفوترة <IP>|<كلمة_المرور>',
    example: '.تثبيت_قالب_الفوترة 192.168.1.1|كلمة_السر',
    isOwner: true,
    isPremium: false,
    isGroup: false,
    isPrivate: false,
...
function handler(m) {
    ...
    if (!text) {
        return m.reply(
            `╭┈┈⬡「 🎨 *تثبيت قالب الفوترة* 」\n┃ ㊗ طريقة الاستخدام: \`${m.prefix}تثبيت_قالب_الفوترة <IP>|<كلمة_المرور>\`\n╰┈┈⬡\n\n> \`مثال: ${m.prefix}تثبيت_قالب_الفوترة 192.168.1.1|كلمة_السر\``
        )
    }
    ...
    conn.on('ready', async () => {
        try {
            ...
            await m.reply(`🕕 *[2/3] تثبيت القالب...*\n\n> جاري تحميل وتثبيت قالب الفوترة...`)
            ...
            m.react('✅')
            await m.reply(
                `╭┈┈⬡「 ✅ *قالب الفوترة* 」\n┃ ㊗ الحالة: *تم التثبيت*\n┃ ㊗ IP: ${ipvps}\n╰┈┈⬡\n\n> _تم تثبيت قالب الفوترة + التبعيات بنجاح!_`
            )
        } catch (err) {
            m.react('☢')
