...
const pluginConfig = {
    ...
    alias: ['leaderboard'],
    category: 'main',
    description: 'عرض قائمة المتصدرين العالميين (كوين، خبرة، طاقة)',
    usage: '.المتصدرين',
    example: '.المتصدرين',
    isOwner: false,
    isPremium: false,
...
async function handler(m, { sock }) {
    ...
    if (users.length === 0) {
        return m.reply(`📊 *المتصدرين*\n\n> لا توجد بيانات مستخدمين مسجلة في قاعدة البيانات.`)
    }
    ...
    if (type === 'overview') {
        ...
        const overviewText = `🏆 *نظرة عامة على المتصدرين* 🏆\n\n` +
            `_اختر زراً أدناه لعرض الترتيب!_`
            try {
                await sock.sendButton(m.chat, fs.readFileSync(path.join(process.cwd(), 'assets', 'images', 'maro.jpg')), overviewText, m, {
                    buttons: [
                    {
                        name: 'quick_reply',
                        buttonParamsJson: JSON.stringify({
                            display_text: '💰 Top كوين',
                            id: `${m.prefix}topkoin`
                        })
                    },
                    {
    ...
    if (type === 'koin') {
        title = 'المتصدرين - الكوينز'
        emoji = '💰'
        field = 'koin'
        formatValue = (u) => `Rp ${formatNumber(u.koin)}`
    } else if (type === 'exp') {
        title = 'المتصدرين - المستوى'
        emoji = '✨'
        field = 'exp'
        formatValue = (u) => `Lv. ${u.level} (${formatNumber(u.exp)} XP)`
    } else if (type === 'energi') {
        title = 'المتصدرين - الطاقة'
        emoji = '⚡'
        field = 'energi'
        formatValue = (u) => `${formatNumber(u.energi)} طاقة`
    }
    ...
    top10.forEach((u, i) => {
        ...
        const isMe = u.jid === senderJid ? " *(أنت)*" : ""
    ...
    if (myRankIndex !== -1) {
        text += `> موقعك: *#${myRankIndex + 1}* من *${formatNumber(users.length)}* مستخدم.`
    } else {
        text += `> أنت غير مسجل في قاعدة البيانات بعد.`
    }
