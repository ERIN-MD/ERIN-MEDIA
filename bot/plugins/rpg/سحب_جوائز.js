...
const pluginConfig = {
  ...
  alias: ["gacha"],
  category: "rpg",
  description: "سحب للحصول على جائزة عشوائية",
  usage: ".سحب_جوائز",
  example: ".سحب_جوائز",
  isOwner: false,
  isPremium: false,
...
async function handler(m, { sock }) {
  ...
  switch (reward.type) {
    case "balance":
      ...
      typeEmoji = "💰";
      rewardText = `+${reward.amount.toLocaleString()} كوين`;
      break;
    case "exp":
      if (!user.rpg) user.rpg = {};
      db.updateExp(m.sender, reward.amount);
      typeEmoji = "⭐";
      rewardText = `+${reward.amount.toLocaleString()} خبرة`;
      break;
    case "limit":
      db.updateLimit(m.sender, reward.amount);
      typeEmoji = "🎫";
      rewardText = `+${reward.amount} حد`;
      break;
    case "jackpot":
      db.updateKoin(m.sender, reward.amount);
      typeEmoji = "💎";
  ...
  let text = `${createGachaAnimation()} *نتيجة السحب*\n\n`;
  text += `╭─────────────╮\n`;
  ...
  text += `*الندرة:* ${rarityColors[reward.rarity]}\n`;
  text += `*الجائزة:* ${typeEmoji} ${rewardText}\n\n`;
  text += `_التبريد: 5 دقائق_`;
